        ...
        <li className={(perRow === 2)?catalogStyles.catalogItemContainer:catalogStyles.catalogItemContainerAlt}>
           ...
           */ }
            ...
            <div className={catalogStyles.catalogItemDescription} style={{marginTop:(perRow===2)?"26px":"24px", gap: (perRow===2)?"":"12px"}}>
                <Link className={catalogStyles.catalogItemCategories} href={`/work/${title.toLowerCase().trim().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, '-')}`}>
                    <p className={[catalogStyles.catalogItemHeader, catalogStyles.catalogItemLink].join(" ")}>
                        {title}
                    </p>
                </Link>
